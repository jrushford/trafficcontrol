...
func SDSSelectQuery() string {
    ...
    d.fq_pacing_rate,
		...
		d.long_desc_2,
		d.max_dns_answers,
		d.mid_header_rewrite,
		COALESCE(d.miss_lat, 0.0),
		COALESCE(d.miss_long, 0.0),
		d.multi_site_origin,
		...
		d.origin_shield,
		d.profile as profile_id,
    profile.name as profile_name,
    profile.description  as profile_description,
		d.protocol,
		d.qstring_ignore,
		d.range_request_handling,
		d.regex_remap,
    ...
    JOIN type ON d.type = type.id
    LEFT JOIN profile ON ds.profile = profile.id
    LEFT JOIN tenant ON d.tenant_id = tn.id
		WHERE d.id in (SELECT deliveryService FROM deliveryservice_server where server = $1)`
	return selectStmt
}
